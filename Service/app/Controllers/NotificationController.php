...
class NotificationController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        // Properly initialize the Email service
        $this->email = \Config\Services::email();

        // Optionally, load the configuration and set the sender details
        $config = config("Email");
    ...
    public function notifyNewBook()
    {
        ...
        $this->email->setMessage(
            "A new book titled '{$data["book_title"]}' has been added to the library!"
        );
        ...
        if ($this->email->send()) {
            ...
            return $this->response
                ->setStatusCode(500)
                ->setJSON(["error" => "Failed to send notification"]);
        }
    }
    ...
    public function notifyLoanCreated()
    {
        ...
        $this->email->setMessage(
            "Your loan with ID '{$data["loan_id"]}' has been successfully created!"
        );
    ...
    public function notifyLoanReturned()
    {
        ...
        $this->email->setMessage(
            "Your loan with ID '{$data["loan_id"]}' has been successfully returned!"
        );
    ...
    public function notifyLoanDue()
    {
        ...
        $this->email->setMessage(
            "Your loan with ID '{$data["loan_id"]}' is nearing its due date: {$data["due_date"]}."
        );
